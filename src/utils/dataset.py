...
from src.config import BLENDSHAPE_NAMES, N_BLENDSHAPES, FEAT_DIM
...
class BlendshapeDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if s["is_synth"]:
            audio_feats = extract_audio_features(s["wav_path"], n_frames=None, use_preprocessing=self.use_preprocessing)
            n_frames = audio_feats.shape[0]
            targets = np.zeros((n_frames, len(BLENDSHAPE_NAMES)), dtype=np.float32)
            ...
            audio_feats = extract_audio_features(s["wav_path"], n_frames, use_preprocessing=self.use_preprocessing)
            if s["ph_path"]:
